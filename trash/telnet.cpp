...
string performAction(string command, string *name, string program_dir){
    command = removeNewlineChars(command);
    // Change to log directory to record messages
    chdir("log");
    const char* cname = name->c_str();
    cout << "Recording: " << command << endl;
    string response = "";
    if(command.length() == 0){
...
void* SocketHandler(void* lp){
    ...
    string MOD = "Welcome to the terminal. Tell me whatever you feel like to.\n I am your friend.\n> ";
